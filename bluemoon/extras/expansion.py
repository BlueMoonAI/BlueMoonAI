...
class BlueMoonExpansion:
    def __init__(self):
        self.tokenizer = AutoTokenizer.from_pretrained(path_bluemoon_expansion)

        positive_words = open(os.path.join(path_bluemoon_expansion, 'positive.txt'),
        ...
        logly.info(f'BlueMoon AI V1 Expansion: Vocab with {len(debug_list)} words.')
        ...
        self.model = AutoModelForCausalLM.from_pretrained(path_bluemoon_expansion)
        self.model.eval()
        ...
        logly.info(f'BlueMoon AI Expansion engine loaded for {load_device}, use_fp16 = {use_fp16}.')
    ...
    def __call__(self, prompt, seed):
        ...
        if self.patcher.current_device != self.patcher.load_device:
            logly.info('BlueMoon AI Expansion loaded by itself.')
