...
import contextlib
import bluemoon.ldm_patched.ldm.modules.attention as attention

from bluemoon.extras.resampler import Resampler
...
clip_vision: bluemoon.ldm_patched.modules.clip_vision.ClipVisionModel = None
...
def load_ip_adapter(clip_vision_path, ip_negative_path, ip_adapter_path):
    ...
    if clip_vision is None and isinstance(clip_vision_path, str):
        clip_vision = bluemoon.ldm_patched.modules.clip_vision.load(clip_vision_path)
...
def preprocess(img, ip_adapter_path):
    ...
    bluemoon.ldm_patched.modules.model_management.load_model_gpu(clip_vision.patcher)
    ...
    with precision_scope(bluemoon.ldm_patched.modules.model_management.get_autocast_device(clip_vision.load_device), torch.float32):
    ...
    bluemoon.ldm_patched.modules.model_management.load_model_gpu(image_proj_model)
    cond = image_proj_model.model(cond).to(device=ip_adapter.load_device, dtype=ip_adapter.dtype)

    bluemoon.ldm_patched.modules.model_management.load_model_gpu(ip_layers)
