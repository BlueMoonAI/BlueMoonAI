import bluemoon.ldm_patched.modules.supported_models
...
def model_config_from_unet_config(unet_config):
    for model_config in bluemoon.ldm_patched.modules.supported_models.models:
        if model_config.matches(unet_config):
            return model_config(unet_config)

    logly.warn("no match", unet_config)
    return None
...
def model_config_from_unet(state_dict, unet_key_prefix, dtype, use_base_if_no_match=False):
    ...
    if model_config is None and use_base_if_no_match:
        return bluemoon.ldm_patched.modules.supported_models_base.BASE(unet_config)
    else:
        return model_config
