...
class SDClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cpu", max_length=77,
                 freeze=True, layer="last", layer_idx=None, textmodel_json_config=None, dtype=None, model_class=bluemoon.ldm_patched.modules.clip_model.CLIPTextModel,
        ...
        self.transformer = model_class(config, dtype, device, bluemoon.ldm_patched.modules.ops.manual_cast)
            ...
            for y in x:
                if isinstance(y, int):
                    ...
                    if y.shape[0] == current_embeds.weight.shape[1]:
                        ...
                        logly.warn("WARNING: shape mismatch when trying to apply embedding, embedding will be ignored", y.shape[0], current_embeds.weight.shape[1])
...
def load_embed(embedding_name, embedding_directory, embedding_size, embed_key=None):
    ...
    try:
        ...
        print()
        logly.error("error loading embedding, skipping loading:", embedding_name)
        return None
        ...
        for weighted_segment, weight in parsed_weights:
            ...
            for word in to_tokenize:
                ...
                if word.startswith(self.embedding_identifier) and self.embedding_directory is not None:
                    ...
                    if embed is None:
                        logly.warn(f"warning, embedding:{embedding_name} does not exist, ignoring")
                    else:
