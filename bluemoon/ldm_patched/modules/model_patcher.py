...
class ModelPatcher:
    ...
    def model_size(self):
        ...
        self.size = bluemoon.ldm_patched.modules.model_management.module_size(self.model)
    ...
    def get_key_patches(self, filter_prefix=None):
        bluemoon.ldm_patched.modules.model_management.unload_model_clones(self)
    ...
    def patch_model(self, device_to=None):
        ...
        for key in self.patches:
            ...
            if device_to is not None:
                temp_weight = bluemoon.ldm_patched.modules.model_management.cast_to_device(weight, device_to, torch.float32, copy=True)
            else:
                temp_weight = weight.to(torch.float32, copy=True)
            out_weight = self.calculate_weight(self.patches[key], temp_weight, key).to(weight.dtype)
            if inplace_update:
                bluemoon.ldm_patched.modules.utils.copy_to_param(self.model, key, out_weight)
            else:
                bluemoon.ldm_patched.modules.utils.set_attr(self.model, key, out_weight)
            del temp_weight
    ...
    def unpatch_model(self, device_to=None):
        ...
        if self.weight_inplace_update:
            for k in keys:
                bluemoon.ldm_patched.modules.utils.copy_to_param(self.model, k, self.backup[k])
        else:
            for k in keys:
                bluemoon.ldm_patched.modules.utils.set_attr(self.model, k, self.backup[k])
