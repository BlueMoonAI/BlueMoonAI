...
def normalize_key(k):
    ...
    k = ' '.join(words)
    k = k.replace('3d', '3D')
    k = k.replace('nova', 'Nova')
    k = k.replace('mesh', 'Mesh')
    k = k.replace('(s', '(S')
    return k
...
for x in ['sdxl_styles_bluemoon.json',
          'sdxl_styles_nova.json',
          'sdxl_styles_Mesh.json',
          'sdxl_styles_tor.json',
          'sdxl_styles_don.json']:
...
legal_style_names = [bluemoon_expansion] + style_keys
...
def apply_arrays(text, index):
    ...
    logly.info(f'[Arrays] processing: {text}')
    mult = 1
    for arr in arrays:
        words = arr.split(',')
        mult *= len(words)
