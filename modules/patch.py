        ...
        if patch_type == "diff":
            ...
            if alpha != 0.0:
                if w1.shape != weight.shape:
                    logly.info("WARNING SHAPE MISMATCH {} WEIGHT NOT MERGED {} != {}".format(key, w1.shape, weight.shape))
                else:
            ...
            if alpha != 0.0:
                if w1.shape != weight.shape:
                    logly.warn("WARNING SHAPE MISMATCH {} BlueMoonAI WEIGHT NOT MERGED {} != {}".format(key, w1.shape, weight.shape))
                else:
                    weight += alpha * bluemoon.ldm_patched.modules.model_management.cast_to_device(w1, weight.device, weight.dtype)
...
def patched_load_models_gpu(*args, **kwargs):
    ...
    if moving_time > 0.1:
        logly.info(f'[BlueMoonAI Model Management] Moving model(s) has taken {moving_time:.2f} seconds')
    return y
            ...
            for path in list(args) + list(kwargs.values()):
                if isinstance(path, str):
                    if os.path.exists(path):
                        ...
                        if os.path.exists(path):
                            os.remove(path)
                        exp += f'BlueMoonAI has tried to move the corrupted file to {corrupted_backup_file} \n'
                        exp += f'You may try again now and BlueMoonAI will download models again. \n'
...
def patch_all():
    if bluemoon.ldm_patched.modules.model_management.directml_enabled:
        bluemoon.ldm_patched.modules.model_management.lowvram_available = True
        bluemoon.ldm_patched.modules.model_management.OOM_EXCEPTION = Exception
