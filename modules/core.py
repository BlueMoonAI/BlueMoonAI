...
import bluemoon.ldm_patched.modules.latent_formats
import modules.advanced_parameters

from bluemoon.ldm_patched.modules.sd import load_checkpoint_guess_config
from bluemoon.ldm_patched.contrib.external import VAEDecode, EmptyLatentImage, VAEEncode, VAEEncodeTiled, \
    VAEDecodeTiled, \
    ControlNetApplyAdvanced
from bluemoon.ldm_patched.contrib.external_freelunch import FreeU_V2
from bluemoon.ldm_patched.modules.sample import prepare_mask
from modules.lora import match_lora
from bluemoon.ldm_patched.modules.lora import model_lora_keys_unet, model_lora_keys_clip
from modules.config import path_embeddings
from bluemoon.ldm_patched.contrib.external_model_advanced import ModelSamplingDiscrete
...
class StableDiffusionModel:
    ...
    def refresh_loras(self, loras):
        ...
        logly.info(f'Request to load LoRAs {str(loras)} for model [{self.filename}].')
        ...
        for name, weight in loras:
            ...
            if not os.path.exists(lora_filename):
                logly.error(f'Lora file not found: {lora_filename}')
                continue
        ...
        for lora_filename, weight in loras_to_load:
            lora_unmatch = bluemoon.ldm_patched.modules.utils.load_torch_file(lora_filename, safe_load=False)
            ...
            if len(lora_unmatch) > 0:
                logly.info(f'Loaded LoRA [{lora_filename}] for model [{self.filename}] '
                      f'with unmatched keys {list(lora_unmatch.keys())}')

            if self.unet_with_lora is not None and len(lora_unet) > 0:
                loaded_keys = self.unet_with_lora.add_patches(lora_unet, weight)
                logly.info(f'Loaded LoRA [{lora_filename}] for UNet [{self.filename}] '
                      f'with {len(loaded_keys)} keys at weight {weight}.')
                for item in lora_unet:
                    if item not in loaded_keys:
                        logly.info("UNet LoRA key skipped: ", item)

            if self.clip_with_lora is not None and len(lora_clip) > 0:
                loaded_keys = self.clip_with_lora.add_patches(lora_clip, weight)
                logly.info(f'Loaded LoRA [{lora_filename}] for CLIP [{self.filename}] '
                      f'with {len(loaded_keys)} keys at weight {weight}.')
                for item in lora_clip:
                    if item not in loaded_keys:
                        logly.info("CLIP LoRA key skipped: ", item)
...
def load_controlnet(ckpt_filename):
    return bluemoon.ldm_patched.modules.controlnet.load_controlnet(ckpt_filename)
...
def get_previewer(model):
    ...
    is_sdxl = isinstance(model.model.latent_format, bluemoon.ldm_patched.modules.latent_formats.SDXL)
    ...
    if vae_approx_filename in VAE_approx_models:
        ...
        if bluemoon.ldm_patched.modules.model_management.should_use_fp16():
        ...
        VAE_approx_model.to(bluemoon.ldm_patched.modules.model_management.get_torch_device())
...
def ksampler(model, positive, negative, latent, seed=None, steps=30, cfg=7.0, sampler_name='dpmpp_2m_sde_gpu',
    ...
    if sigmas is not None:
        sigmas = sigmas.clone().to(bluemoon.ldm_patched.modules.model_management.get_torch_device())
    ...
    bluemoon.ldm_patched.modules.samplers.sample = modules.sample_hijack.sample_hacked

    try:
        samples = bluemoon.ldm_patched.modules.sample.sample(model,
