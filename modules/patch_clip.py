...
def patched_SDClipModel__init__(self, max_length=77, freeze=True, layer="last", layer_idx=None,
    ...
    if textmodel_json_config is None:
        textmodel_json_config = os.path.join(os.path.dirname(os.path.realpath(bluemoon.ldm_patched.modules.sd1_clip.__file__)),
...
def patched_ClipVisionModel__init__(self, json_config):
    ...
    self.load_device = bluemoon.ldm_patched.modules.model_management.text_encoder_device()
    self.offload_device = bluemoon.ldm_patched.modules.model_management.text_encoder_offload_device()

    if bluemoon.ldm_patched.modules.model_management.should_use_fp16(self.load_device, prioritize_performance=False):
    ...
    self.patcher = bluemoon.ldm_patched.modules.model_patcher.ModelPatcher(
        self.model,
...
def patched_ClipVisionModel_encode_image(self, image):
    bluemoon.ldm_patched.modules.model_management.load_model_gpu(self.patcher)
    pixel_values = bluemoon.ldm_patched.modules.clip_vision.clip_preprocess(image.to(self.load_device))
    ...
    with precision_scope(bluemoon.ldm_patched.modules.model_management.get_autocast_device(self.load_device), torch.float32):
    ...
    for k in outputs:
        ...
        if t is not None:
            if k == 'hidden_states':
                outputs["penultimate_hidden_states"] = t[-2].to(bluemoon.ldm_patched.modules.model_management.intermediate_device())
                outputs["hidden_states"] = None
            else:
                outputs[k] = t.to(bluemoon.ldm_patched.modules.model_management.intermediate_device())

    return outputs


def patch_all_clip():
    bluemoon.ldm_patched.modules.sd1_clip.ClipTokenWeightEncoder.encode_token_weights = patched_encode_token_weights
    bluemoon.ldm_patched.modules.sd1_clip.SDClipModel.__init__ = patched_SDClipModel__init__
    bluemoon.ldm_patched.modules.sd1_clip.SDClipModel.forward = patched_SDClipModel_forward
    bluemoon.ldm_patched.modules.clip_vision.ClipVisionModel.__init__ = patched_ClipVisionModel__init__
    bluemoon.ldm_patched.modules.clip_vision.ClipVisionModel.encode_image = patched_ClipVisionModel_encode_image
    return